 ...
 * @return Minkowski distance between x and y of order p.
 */
double minkowski(const Matrix &x, const Matrix &y, int p)
{
  int nrx, nry, ncx, ncy;
  int i, j;
  double sum = 0.0;
  ...
  if (!(nrx*nry==1 || ncx*ncy==1)) {
    cout << "The minkowski() routine can only calculate distance of vectors\n";
    exit(3);
  }
  ...
  for (i=0; i<nrx; i++) // two loops is to make indifference to both row and column vectors
    for (j=0; j<ncx; j++)
      sum += pow(fabs(x(i,j)-y(i,j)),p);
