...
int convertHexToDecimal(char *); // converts hex string to decimal value
int getCacheSetNumber(int , int ); // returns set no. / line no. the address block belongs to for given cache lines
int getWordAddress(int , int ); // returns word address for given byte address and word size
int getBlockAddress( int , int ); // returns block address for given word address and block size
int getcacheTag(int , int ); // returns cache tag for given block address and no. of cache lines
...
int main(int argc, char** argv)
{
	...
	double rDBRRate1, rDBRRate2, rDBRRate3, wDBRRate1, wDBRRate2, wDBRRate3;
	if(writeMethod==1) // write back
	{
		...
		if(noOfCache==1)
		{
			AMATRead=hitTime1+rMissRate1*(memAccessTime + hitTime1 + rDBRRate1*memAccessTime);
			AMATWrite=hitTime1+wMissRate1*(memAccessTime + hitTime1 + wDBRRate1*memAccessTime);
		}
		else if(noOfCache==2)
		{
			rDBRRate2=cache2ReadDirty/(double)cache2ReadMiss;
			wDBRRate2=cache2WriteDirty/(double)cache2WriteMiss;
			AMATRead=hitTime1+rMissRate1*(hitTime2+hitTime1+rDBRRate1*hitTime2+rMissRate2*(memAccessTime+hitTime2+rDBRRate2*memAccessTime));
			AMATWrite=hitTime1+wMissRate1*(hitTime2+hitTime1+wDBRRate1*hitTime2+wMissRate2*(memAccessTime+hitTime2+wDBRRate2*memAccessTime));
		}
		else
		{
			rDBRRate2=cache2ReadDirty/(double)cache2ReadMiss;
			...
			AMATRead=hitTime1+rMissRate1*(hitTime2+hitTime1+rDBRRate1*hitTime2+rMissRate2*(hitTime3+hitTime2+rDBRRate2*hitTime3+rMissRate3*(memAccessTime+hitTime3+rDBRRate3*memAccessTime)));
			AMATWrite=hitTime1+wMissRate1*(hitTime2+hitTime1+wDBRRate1*hitTime2+wMissRate2*(hitTime3+hitTime2+wDBRRate2*hitTime3+wMissRate3*(memAccessTime+hitTime3+wDBRRate3*memAccessTime)));
		}
	}
	else // write through
	{
		if(noOfCache==1)
		{
			AMATRead=hitTime1+rMissRate1*(memAccessTime + hitTime1);
			AMATWrite=hitTime1+memAccessTime+wMissRate1*(memAccessTime + hitTime1);
		}
		else if(noOfCache==2)
		{
			AMATRead=hitTime1+rMissRate1*(hitTime2+hitTime1+rMissRate2*(memAccessTime+hitTime2));
			AMATWrite=hitTime1+hitTime2+memAccessTime+wMissRate1*(hitTime2+hitTime1+wMissRate2*(memAccessTime+hitTime2));
		}
		else
		{
			AMATRead=hitTime1+rMissRate1*(hitTime2+hitTime1+rMissRate2*(hitTime3+hitTime2+rMissRate3*(memAccessTime+hitTime3)));
			AMATWrite=hitTime1+hitTime2+hitTime3+memAccessTime+wMissRate1*(hitTime2+hitTime1+wMissRate2*(hitTime3+hitTime2+wMissRate3*(memAccessTime+hitTime3)));
		}
	}
	...
	cout<<"AMAT : "<<((double)AMATRead*(double)totalRead + (double)AMATWrite*(double)totalWrite)/(double)totalAccess;
