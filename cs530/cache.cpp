...
int setLRUUnitHighest(int **,int,int,int);

int main()
{
	char *fname;
	int wordSize; // common word size in bytes for all levels of cache
	int blockSize1, noOfLines1, assoc1,blockSize2, noOfLines2, assoc2,blockSize3, noOfLines3, assoc3; // individual parameters of cache
	...
	cache1= new int[noOfLines1];
	LRU1=new int[noOfLines1];
	dirty1=new [noOfLines1];
	for(i=0;i<noOfLines1;i++)
	{
	...
	if(noOfCache>1)
	{
		blockSize2=5; noOfLines2=200;
		assoc2=8;	
		// ready structure for cache 2 for no. of cache lines
		cache2= new int[noOfLines2];
		LRU2=new int[noOfLines2];
		dirty2=new [noOfLines2];
		for(i=0;i<noOfLines2;i++)
		{
	...
	if(noOfCache>2)
	{
		blockSize3=6; noOfLines3=400; assoc3=4;
		// ready structure for cache 3 for no. of cache lines
		cache3= new int[noOfLines3];
		LRU3=new int[noOfLines3];
		dirty3=new [noOfLines3];
		for(i=0;i<noOfLines3;i++)
		{
...
int setLRUUnitHighest(int **LRU,int set,int assoc,int pos);
{
    int i,value;
    for(i=0;i<assoc;i++)
        if(LRU[set][value] == pos)
		{
            value = i;
			break;
		}
