...
{  
  ...
  // free block used for entry for the directory which is indexed by 0  
  ...
  boff   = (pinode.fSize - pindex*(BLOCKSIZE/sizeof(DEntry))) - sizeof(DEntry);  //
  ...
  // replace to be removed entry by this entry if it is not last block else it is done
  Directory *d, *c;
  int read, idx, i, off;
  bool found = 0;
  ...
  if(dl.inum != inum){ // if last entry inum not equals to removing entry inum, find entry to be removed
   ...
   if(pindex < IPOINTS){
	...
	// read one by one for all possible blocks and if found break
     ...
     if(found){
       // replace that entry
       fseek(FSP, (pinode.pointer[pindex]+SBlock.first_dblock)*BLOCKSIZE + read-sizeof(DEntry), SEEK_SET);
...
void openf()
{
  ...
  pinum = FSContext.cwd;     // inum of current working directory
  strcpy(name, MY_ARGV[1]);
  if(strcmp(MY_ARGV[2], "1") != 0 && strcmp(MY_ARGV[2], "2") != 0 && strcmp(MY_ARGV[2], "3") != 0){
