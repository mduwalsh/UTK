...
class Message{
	...
	int sltime; // logical time of message sent event
...
void place_message_in_channel(int pTime,int sender, int receiver,int sltime, int delay, char *type)
{
	int i;
	for(i=0;i<CH_LENGTH;i++)
	{
		...
		if(channel[sender][receiver][i].type=="NULL")
		{
			...
			channel[sender][receiver][i].sltime=sltime;
...
void go_snap(int node)
{
	...
	for(i=0;i<nodes;i++)
	{
		if(i!=node)
		{
			place_message_in_channel(snapState[node].pTime,node,ltime, i, 0, "S");
		}
	}
...
int main(int argc, char** argv)
{
	...
	while(infile>>pTime>>op || msgLine==1) // repeat until all inputs are read and there is no messages in any of the channels
	{		
					...
					if(i!=j) // only for channels with non selfnode
					{
						...
						if(channel[j][i][0].type=="G") // for general message
						{
							...
							{	//message received from node j to node i
								//update node logical time
								if(nodeState[i].lTime>channel[j][i][0].sltime)
								{
									nodeState[i].lTime++;
								}
								else
								{
									nodeState[i].lTime=channel[j][i][0].sltime+1;
								}
			...
			if(infile>>p){
				switch (op)
				{
					case 'm': // message send event
						...
						nodeState[p].event='M';
						place_message_in_channel(pTime,p,n,nodeState[p].lTime,t,"G");
						break;
					case 'l': // local event
						infile>>t;
						nodeState[p].lTime++;
